...
type SendVerificationEmailType = (email: string, otp: string) => Promise<undefined>
const sendVerificationEmail: SendVerificationEmailType = async (email, otp) => {
    try {
        const mailResponse: SMTPTransport.SentMessageInfo | undefined = await mailSender(email, "Verification Email from StudyNotion", otp);
